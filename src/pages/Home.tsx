...
const structuredData = {
  ...
  potentialAction: {
    "@type": "SearchAction",
    target: "https://greenglobe.com/search?q={search_term_string}",
...
const Home = () => {
  ...
  const handleSearch = () => {
    if (searchTerm.trim()) navigate(`/usa?search=${encodeURIComponent(searchTerm.trim())}`);
  };
  ...
  return (
    <>
      {/* SEO + structured data */}
      <head>
        <title>Green Globe | Cannabis Travel Guide 2025 – Laws, 420 Hotels, Airports</title>
        <meta name="description" content="Plan safe, legal cannabis trips. Real-time laws, 420-friendly hotels, airport rules for 120+ countries. Attorney-verified, updated daily." />
        <link rel="canonical" href="https://greenglobe.com" />
      ...
      <div className="min-h-screen bg-background overflow-x-hidden selection:bg-accent/30">
        ...
        <section className="py-20 px-4 bg-gradient-to-t from-background to-black relative overflow-hidden">
          ...
          >
            <h2 className="text-4xl md:text-6xl font-bold mb-6">Ready to Travel?</h2>
            <p className="text-xl text-muted-foreground mb-10 max-w-2xl mx-auto">
              Get instant access to verified cannabis laws and 420-friendly hotels worldwide.
            </p>
            <Link to="/usa">
              <Button size="lg" className="h-16 px-10 text-xl bg-accent hover:bg-accent/90 text-accent-foreground font-semibold shadow-glow rounded-2xl hover:scale-105 transition-transform">
                Start Exploring Now
              </Button>
            </Link>
          </motion.div>
