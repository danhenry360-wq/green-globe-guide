...
/* ----------------------------------------------------
   DATA – dedupe countries to fix Canada / Netherlands issue
----------------------------------------------------- */

// Remove duplicate countries by slug or name
const UNIQUE_DATA: CountryHotels[] = Array.from(
  new Map(
    HOTEL_DATA.map((c) => [c.slug || c.country.toLowerCase(), c])
  ).values()
);
...
const HOTELS_STRUCTURED_DATA = {
  ...
  mainEntity: UNIQUE_DATA.map((country) => ({
    "@type": "Place",
...
const Hotels = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    const allHotels = UNIQUE_DATA.flatMap((country) =>
  ...
  return (
    <>
      <head>
        ...
        <meta
          name="description"
          content="Book BudQuest-verified 420-friendly hotels by country and state. Cannabis policies checked, star-rated, premium stays."
        />
          ...
          <div className="container mx-auto max-w-7xl">
            ...
            <section className="max-w-3xl mx-auto mb-12 text-center">
              ...
              <p className="text-sm sm:text-base text-muted-foreground">
                Browse by country or state. Policies checked, premium experience.
              </p>
            </section>
            ...
            <AnimatePresence mode="wait">
              {filteredData.length === 0 && (
                ...
                >
                  ...
                  <p className="text-lg">
                    No verified 420-friendly hotels match your search query.
                  </p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <div className="space-y-10">
              {hierarchicalHotels.map((countryGroup) => (
                <Collapsible
                  ...
                  className="border border-border rounded-xl shadow-lg bg-card/50"
                >
                  <CollapsibleTrigger className="flex items-center justify-between w-full p-4 md:p-6 hover:bg-card transition-colors rounded-t-xl">
                    <div className="flex items-center gap-3">
                      <img
                        ...
                        className="h-6 w-8 rounded border border-border shadow-md"
                      />
                            ...
                            <div key={cityGroup.city} className="space-y-3">
                              <h4 className="text-base font-bold text-white flex items-center gap-2">
                                ...
                                <Badge className="bg-gold/10 text-gold border border-gold/30 text-xs">
                                  {cityGroup.hotels.length} Hotels
                                </Badge>
                              </h4>
                ...
                <CollapsibleContent>
                  <Card className="p-4 bg-card/50 border-border mt-2">
                    <p className="text-xs text-red-400">
                      BudQuest is an informational resource only. Always verify
                      local laws and hotel policies before booking.
                    </p>
                  </Card>
                </CollapsibleContent>
              </Collapsible>
