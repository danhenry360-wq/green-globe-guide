...
const HOTELS: Hotel[] = [
  ...
  { id: 7, name: "CannaComfort Suites", city: "Las Vegas", state: "NV", rating: 4.6, policies: "Dedicated 420 rooms. Nightly entertainment included.", website: "https://example.com", flag: "https://flagcdn.com/w40/us.png" },
  { id: 8, name: "Herbal Haven Hotel", city: "San Diego", state: "CA", rating: 4.7, policies: "Eco-friendly, rooftop smoking area. Complimentary yoga classes.", website: "https://example.com", flag: "https://flagcdn.com/w40/us.png" },
  { id: 9, name: "The Rolling Leaf Inn", city: "Boulder", state: "CO", rating: 4.8, policies: "Pet-friendly, cannabis concierge, local tours.", website: "https://example.com", flag: "https://flagcdn.com/w40/us.png" },
  { id: 10, name: "Highland Retreat", city: "Austin", state: "TX", rating: 4.5, policies: "420-friendly suites, pool access, music lounge.", website: "https://example.com", flag: "https://flagcdn.com/w40/us.png" },
  { id: 11, name: "Elevate Hotel", city: "Miami", state: "FL", rating: 4.6, policies: "Private balconies, rooftop lounge, wellness services.", website: "https://example.com", flag: "https://flagcdn.com/w40/us.png" },
  { id: 12, name: "Bud & Stay Resort", city: "Chicago", state: "IL", rating: 4.7, policies: "On-site dispensary, rooftop lounge, event spaces.", website: "https://example.com", flag: "https://flagcdn.com/w40/us.png" },
];
...
const structuredData = {
  ...
  mainEntityOfPage: { "@type": "WebPage", "@id": "https://greenglobe.com/hotels" },
  potentialAction: { "@type": "SearchAction", target: "https://greenglobe.com/hotels?search={query}", "query-input": "required name=query" },
...
const Star = ({ filled }: { filled: boolean }) => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" className={`w-4 h-4 ${filled ? "fill-yellow-400 text-yellow-400" : "text-gray-600"}`}>
...
const Hotels = () => {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return HOTELS;
    return HOTELS.filter(h => h.name.toLowerCase().includes(q) || h.city.toLowerCase().includes(q) || h.state.toLowerCase().includes(q));
  }, [query]);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-background text-white">
        ...
        {/* STATIC SEARCH BAR */}
        <div className="fixed top-[80px] left-0 w-full z-50 px-4">
          <div className="max-w-7xl mx-auto">
            <div className="bg-background/90 backdrop-blur-md rounded-lg border border-border p-3">
        ...
        {/* CONTENT */}
        <main className="pt-[160px] pb-20 px-4">
          <section className="container mx-auto max-w-7xl">
            {/* HERO */}
            <header className="max-w-3xl mx-auto mb-10 text-center">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-3">BudQuest Verified Hotels</h1>
              <p className="text-sm sm:text-base text-muted-foreground">420-friendly stays worldwide. Policies verified. Premium experience.</p>
            </header>

            {/* STATE LINKS */}
            <nav className="flex flex-wrap gap-2 justify-center mb-8">
              {states.map(state => (<a key={state} href={`#state-${state}`} className="text-accent hover:text-accent-foreground text-sm font-medium">{state}</a>))}
            </nav>
                ...
                <section key={state} id={`state-${state}`} className="mb-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {hotelsInState.map(hotel => (
                      <article key={hotel.id} className="overflow-hidden hover:shadow-xl transition-shadow rounded-xl flex flex-col border border-border bg-card/50">
                        <div className="aspect-video relative bg-gradient-to-br from-accent/10 to-accent/5 flex items-center justify-center">
                          <img src={hotel.flag} alt={`Flag of ${hotel.state}`} loading="lazy" className="h-6 w-8 rounded border border-border absolute top-3 left-3"/>
                        ...
                        <div className="p-5 flex flex-col gap-3 flex-1">
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <Badge className="bg-green-600 text-white text-xs flex items-center gap-1"><Leaf className="w-3 h-3"/> Verified</Badge>
                          </div>
                          <p className="text-sm text-muted-foreground">{hotel.city}, {hotel.state}</p>
                          <StarRating value={hotel.rating} />
                          {hotel.policies && <p className="text-sm text-muted-foreground line-clamp-3 flex-1">{hotel.policies}</p>}
                          <a href={hotel.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-accent hover:text-accent-foreground mt-auto">
            ...
            <section className="mt-12 p-4 bg-card/50 rounded-lg text-xs text-muted-foreground">
              <p><strong>Disclaimer:</strong> All hotels listed are BudQuest-verified for cannabis-friendly policies. Users must comply with local laws regarding cannabis use. Green Globe is not responsible for policy changes or violations.</p>
            </section>
