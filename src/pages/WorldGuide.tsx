...
export default WorldGuide;  optimize this without chnaging logic and put last updated date today's date
Model
Thinking Thoughts

Expand to view model thoughts
chevron_right
To optimize this file for SEO and performance without changing the business logic or navigation structure, I have applied the following changes:

1.  **Dynamic SEO Metadata**: The `<Helmet>` now generates specific Title Tags and Meta Descriptions based on the current view (Continent, Country, or City). For example, viewing Japan will generate a title like *"Is Weed Legal in Japan? 2025 Laws & Penalties"*.
2.  **Schema Markup Enhancement**: Added `dateModified` to the structured data to signal content freshness to Google.
3.  **Semantic HTML**: Replaced generic `div` wrappers with `<main>`, `<header>`, `<article>`, and `<section>` to help search engines understand the page structure.
4.  **Performance Memoization**: Optimized the search filtering functions (`getAllCountries`) using `useMemo` so they don't recalculate on every render, which improves responsiveness on mobile devices.
5.  **"Last Updated" Date**: Added a dynamic date stamp set to **today** (formatted) to display freshness to users.
...
// OPTIMIZATION: Define current date constant for SEO freshness
...
const CountryIndex = ({ continent }: { continent: Continent }) => {
  ...
  return (
    <>
      {/* Fixed Search Bar */}
      <header className="fixed top-16 left-0 right-0 z-40 bg-background/95 backdrop-blur-xl border-b border-white/10">
        <div className="container mx-auto max-w-7xl px-4 py-3">
          <div className="relative max-w-3xl mx-auto">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
            <input
      ...
      <section className="container mx-auto max-w-7xl px-4 pt-40 pb-12">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground">Choose a country to view laws, penalties, and tips.</p>
        </motion.div>
    ...
    <>
      ...
      <section className="container mx-auto max-w-7xl px-4 pt-40 pb-12">
        ...
        >
          ...
          <article className="mt-4 p-4 rounded-lg bg-card border border-white/10 max-w-3xl">
            <h3 className="font-semibold mb-2 flex items-center gap-2">
              <Info className="w-4 h-4 text-primary" /> Legal Context
            </h3>
            <p className="text-muted-foreground mb-4">{country.description}</p>
...
const CityIndex = ({ continent, country, region }: { continent: Continent; country: Country; region: Region }) => {
  ...
  return (
    <>
      <section className="container mx-auto max-w-7xl px-4 pt-32 pb-12">
        <Button
          variant="ghost"
          onClick={() => nav(`/world/${continent.slug}/${country.slug}`)}
          className="mb-6 gap-2"
      ...
      <div className="container mx-auto max-w-5xl px-4 pt-32 pb-12">
        ...
        >
          <article>
            <Card className={cn("rounded-2xl border border-white/10", "bg-gradient-to-br from-green-400/10 via-transparent to-green-400/5 p-6 shadow-lg")}>
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-2">
                <h1 className="text-3xl font-bold">{city.name} Cannabis Guide</h1>
                <div className="flex gap-2">
                  <Badge className={`${statusColor(country.legalStatus)} border-none w-fit`}>
                    {country.legalStatus}
                  </Badge>
